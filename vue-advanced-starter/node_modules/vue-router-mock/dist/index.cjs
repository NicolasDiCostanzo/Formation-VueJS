...
function createReactiveRouteLocation(route) {
  return vue.reactive(
    Object.keys(route.value).reduce(
      (newRoute, key) => {
        newRoute[key] = vue.computed(() => route.value[key]);
        return newRoute;
      },
      {}
    )
...
const EmptyView = vue.defineComponent({
...
function createRouterMock(options = {}) {
  ...
  const initialLocation = options.initialLocation || vueRouter.START_LOCATION;
  ...
  function reset() {
    ...
    onErrorRemovers = [];
    nextReturn = void 0;
    router.currentRoute.value = initialLocation === vueRouter.START_LOCATION ? vueRouter.START_LOCATION : router.resolve(initialLocation);
  }
...
function injectRouterMock(router) {
  ...
  const route = provides[vueRouter.routeLocationKey];
  Object.assign(testUtils.config.global.provide, provides);
  testUtils.config.global.mocks.$router = router;
  testUtils.config.global.mocks.$route = route;
  testUtils.config.global.components.RouterView = vueRouter.RouterView;
  testUtils.config.global.components.RouterLink = vueRouter.RouterLink;
  testUtils.config.global.stubs.RouterLink = true;
  testUtils.config.global.stubs.RouterView = true;
  return { router, route };
}
function createProvide(router) {
  ...
  return {
    [vueRouter.routerKey]: router,
    [vueRouter.routeLocationKey]: route,
    [vueRouter.routerViewLocationKey]: router.currentRoute,
...
function getRouter() {
  return testUtils.config.global.provide[vueRouter.routerKey];
}

exports.EmptyView = EmptyView;
exports.VueRouterMock = plugin;
exports.createProvide = createProvide;
exports.createRouterMock = createRouterMock;
exports.getRouter = getRouter;
exports.injectRouterMock = injectRouterMock;
